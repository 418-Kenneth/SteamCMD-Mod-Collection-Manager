    ...
    public class Get_Modcollection
    {
        ...
        public async Task<List<string>> workshopids(string id)
        {
            try
            {
                ...
                var result = data.Split(stringSeparators, StringSplitOptions.None).ToList<string>().Where(p => p.Contains("SharedFileBindMouseHover")).Select(s => s.Remove(0, 42)).Select(b => b.Remove(b.Length - 1, 1)).ToList();
